...
public class Pack_4_Streams_Difficult {
    ...
    public void exercise_1_findFirst() {
        ...
        String result = EMPLOYEES.stream().filter(e1 -> EMPLOYEES.stream()
                        .anyMatch(e2 -> e1.getFirstName().equals(e2.getFirstName()) && e1.getSurname().equals(e2.getSurname()) && !e1.equals(e2)))
    ...
    public void exercise_2_groupingBy_counting() {
        ...
        long result = EMPLOYEES.stream()
                .collect(Collectors.groupingBy(e -> e.getHomeAddress().getPostCode().substring(0, 2), Collectors.toList()))
                .values()
                .stream()
                .filter(e -> e.size() >= 5)
                .count();
    ...
    public void exercise_3_flatMap() {
        ...
        long result = EMPLOYEES.stream().flatMap(e -> Stream.of(e.getHomeAddress(), e.getCorrespondenceAddress().get()))
    ...
    public void exercise_5_patternCompileSplitAsStream() {
        ...
        List<String> result = Arrays.stream(string.split("\n"))
    ...
    public void exercise_7_randomLongs_concat_toArray() {
        ...
        int[] result = LongStream.concat(longs, ints.mapToLong(Long::valueOf)) //longs.mapToInt(value -> (int) value),ints
                .map(Math::abs)
                .sorted()
                .skip(5)
                .limit(10)
